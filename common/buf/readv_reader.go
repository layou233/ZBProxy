package buf

import (
	"io"
	"net"
	"syscall"
)

type allocStrategy struct {
	current uint32
}

func (s *allocStrategy) Current() uint32 {
	return s.current
}

func (s *allocStrategy) Adjust(n uint32) {
	if n >= s.current {
		s.current *= 2
	} else {
		s.current = n
	}

	if s.current > 8 {
		s.current = 8
	}

	if s.current == 0 {
		s.current = 1
	}
}

func (s *allocStrategy) Alloc() net.Buffers {
	bs := make(net.Buffers, s.current)
	for i := range bs {
		bs[i] = Get(8 * 1024)
	}
	return bs
}

type vectorizedReader interface {
	Init(buffers net.Buffers)
	Read(fd uintptr) int32
	Clear()
}

// ReaderV is a Reader that uses readv(2) syscall to read data.
type ReaderV struct {
	io.Reader
	rawConn syscall.RawConn
	mr      vectorizedReader
	alloc   allocStrategy
}

// NewReaderV creates a new ReaderV.
func NewReaderV(reader io.Reader, rawConn syscall.RawConn) *ReaderV {
	return &ReaderV{
		Reader:  reader,
		rawConn: rawConn,
		alloc: allocStrategy{
			current: 1,
		},
		mr: newVectorizedReader(),
	}
}

func (r *ReaderV) readMulti() (net.Buffers, error) {
	bs := r.alloc.Alloc()

	r.mr.Init(bs)
	var nBytes int32
	err := r.rawConn.Read(func(fd uintptr) bool {
		n := r.mr.Read(fd)
		if n < 0 {
			return false
		}

		nBytes = n
		return true
	})
	r.mr.Clear()

	if err != nil {
		PutMulti(bs)
		return nil, err
	}

	if nBytes == 0 {
		PutMulti(bs)
		return nil, io.EOF
	}

	nBuf := 0
	for nBuf < len(bs) {
		if nBytes <= 0 {
			break
		}
		end := nBytes
		if end > 8*1024 {
			end = 8 * 1024
		}
		bs[nBuf] = bs[nBuf][:end]
		nBytes -= end
		nBuf++
	}

	for i := nBuf; i < len(bs); i++ {
		Put(bs[i]) //nolint:errcheck
		bs[i] = nil
	}

	return bs[:nBuf], nil
}

func (r *ReaderV) ReadVectorized() (net.Buffers, error) {
	mb, err := r.readMulti()
	if err != nil {
		return nil, err
	}
	r.alloc.Adjust(uint32(len(mb)))
	return mb, nil
}
